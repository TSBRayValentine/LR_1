...
void profile_matrix_times_matrix(int n, OpenCL& opencl) {
    ...
    opencl.queue.enqueueNDRangeKernel(kernel, cl::NullRange, cl::NDRange(n, n), cl::NDRange(1024));
    opencl.queue.flush();
...
kernel void matrix_times_matrix(global const float* a,
    ...
    const int i_l = get_local_id(0);
    const int n_l = get_local_size(0);

 __local float local_buffer[1024]; 

   
    for (int k = i_l; k < n; k += n_l) {
        local_buffer[k] = a[j * n + k]; 
    }
